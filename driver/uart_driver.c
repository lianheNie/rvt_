...
static PIN_Config _ioPinTable[] = {
    _AW_RX_PIN_ID | PIN_GPIO_OUTPUT_EN | PIN_GPIO_HIGH | PIN_PUSHPULL |
        PIN_DRVSTR_MAX,
    _AW_TX_PIN_ID | PIN_GPIO_OUTPUT_EN | PIN_GPIO_HIGH | PIN_PUSHPULL |
        PIN_DRVSTR_MAX,
    PIN_TERMINATE};
...
s8 uart_printf_add(char **buf, const char *__restrict _format, ...) {
  ...
  va_start(args, _format);
  int cnt =
      vsnprintf(&_uart_tx_buf[len], _UART_RX_BUF_SIZE - len, _format, args);
  va_end(args);
