...
static u16 aw_to_millivolts(int32_t adcValue) {
  adcValue = AUXADCAdjustValueForGainAndOffset(
...
#define _POWER_ADC_RATIO 6.12f      // 2.03     // ADC 电阻分压系数
...
bool aw_power_adc_update(u16 *mV) {
  ...
  u16 milliVolt =
      aw_to_millivolts(scifTaskData.adcWindowMonitor.output.adcValue);
  ...
  if (_is_adcFirst) {
    ...
    if (_is_adcFirstCnt > 5) {
      ...
      aw_delay_ms(1);
      scifInit(&scifDriverSetup);
      scifStartRtcTicksNow(65536 * 30 * 60);  // 2*60s
      scifStartTasksNbl(BV(SCIF_ADC_WINDOW_MONITOR_TASK_ID));
    }
  }
  return false;
