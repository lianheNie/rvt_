...
static aw_at_status_t _status = {.mqtt_sts = MQTT_STS_FREE,
                                 .is_building = false};
#endif
...
const aw_at_urc_t *aw_at_check_urc(const aw_uart_rx_t *uart_buf) {
  ...
  for (i = 0; i < Urc_CNT; i++) {
    ...
    if ((prefix_len ? !strncmp(buffer, urc->cmd_prefix, prefix_len) : 1) &&
        (suffix_len ? !strncmp(buffer + bufsz - suffix_len, urc->cmd_suffix,
                               suffix_len)
                    : 1)) {
...
#endif
static char _payload[_AW_AT_PAYLOAD_SIZE] = {
    '\0'};  //字符串缓存,用于数据发送和参数化配置
...
int aw_mqtt_set_ate(aw_at_ate_t ate) {
  ...
  at_resp_status_t rsp_sts =
      aw_at_exec_cmd(AT_DEFAULT_TIMEOUT, "ATE%d\r\n", (int)ate);
  return rsp_sts;
}
...
static int aw_mqtt_set_alive(u32 alive_time) {
  ...
  aw_at_response_set_line(AW_MQTT_OK_LINE_NUM);
  at_resp_status_t rsp_sts =
      aw_at_exec_cmd(AT_DEFAULT_TIMEOUT, "" AT_MQTT_ALIVE "" AT_END_CR_LF "",
...
static int aw_mqtt_reboot() {
  ...
  aw_at_response_set_line(AW_MQTT_OK_LINE_NUM);
  at_resp_status_t rsp_sts =
      aw_at_exec_cmd(AT_DEFAULT_TIMEOUT, "" AT_REBOOT "" AT_END_CR_LF "");
#ifdef _IS_USE_STATIS
  _statis.reset_cnt++;
#endif
#ifdef _IS_USE_STATUS
...
static int aw_mqtt_set_will(aw_qos_t qos, u8 retain, const char *msg,
  ...
  aw_at_response_set_line(AW_MQTT_OK_LINE_NUM);
  at_resp_status_t rsp_sts =
      aw_at_exec_cmd(AT_DEFAULT_TIMEOUT, "" AT_MQTT_WILL "" AT_END_CR_LF "",
                     ConnectID_0, qos, retain, topic, msg);
#ifdef _IS_USE_STATUS
  _status.mqtt_sts = MQTT_STS_FREE;
#endif
  return rsp_sts;
...
static int aw_mqtt_open(const char *hostName, u32 port) {
  ...
  aw_at_response_set_line(AW_MQTT_SUCC_LINE_NUM);
  at_resp_status_t rsp_sts =
      aw_at_exec_cmd(AT_DEFAULT_TIMEOUT, "AT+QMTOPEN=%d,\"%s\",%d\r\n",
...
static int aw_mqtt_connect(const char *clientID) {
  ...
  aw_at_response_set_line(AW_MQTT_SUCC_LINE_NUM);
  at_resp_status_t rsp_sts =
      aw_at_exec_cmd(AT_LONG_TIMEOUT, "" AT_MQTT_CONNECT "" AT_END_CR_LF "",
                     ConnectID_0, clientID);
...
static int aw_mqtt_sub(aw_qos_t qos, const char *topic) {
  ...
  aw_at_response_set_line(AW_MQTT_SUCC_LINE_NUM);
  at_resp_status_t rsp_sts =
      aw_at_exec_cmd(AT_DEFAULT_TIMEOUT, "" AT_MQTT_SUB "" AT_END_CR_LF "",
                     ConnectID_0, MsgID_1, topic, qos);

  if (AT_RESP_OK == rsp_sts) {
    if (AW_OK != aw_at_response_check_key(AT_MQTT_SUB_SUCC, ConnectID_0,
...
static s8 aw_mqtt_pub(aw_qos_t qos, u8 retain, const char *topic,
  ...
  if (AT_RESP_OK == rsp_sts) {
    ...
    if (AT_RESP_OK == rsp_sts) {
      if (AW_OK != aw_at_response_check_key(AT_MQTT_PUB_SUCC, ConnectID_0,
...
int aw_mqtt_pub_status() {
  ...
  res = aw_mqtt_pub_f(
      ...
      _statis.pub_err_cnt, aw_mqtt_get_csq(), AW_DEV_TYPE);
//#ifdef IS_USE_OLED
