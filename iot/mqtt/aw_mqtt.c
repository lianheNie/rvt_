...
static aw_mqtt_config_t _config = {
...
void        aw_mqtt_set_sub(u8 is_sub) { _is_sub = is_sub; }
const char *aw_mqtt_get_host() { return Host; }
void        aw_mqtt_set_keepAlive(u16 keepAliveTime) {
    KeepAliveTime = keepAliveTime;
    aw_mqtt_set_alive(KeepAliveTime);
}
const aw_mqtt_config_t *aw_mqtt_config_get() { return &_config; }
void                    aw_mqtt_config_set_qos(aw_qos_t qos) { _config.qos = qos; }

void      aw_mqtt_config_set_host(const char *host) { _config.host = host; }
void      aw_mqtt_config_set_dev_name(const char *dev_name) { _config.dev_name = dev_name; }
void      aw_mqtt_config_set_dev_id(const char *dev_id) { _config.dev_id = dev_id; }
...
static aw_at_response_t _at_resp = {.buf      = {'\0'},
                                    .key      = {'\0'},
...
aw_at_response_t *aw_at_response_get() { return &_at_resp; }
...
static aw_at_staist_t _statis = {.at_rsp_err_cnt  = 0,
                                 .connect_ok_cnt  = 0,
                                 .connect_cnt     = 0,
                                 .at_timeout_cnt  = 0,
                                 .pub_err_cnt     = 0,
...
static aw_at_status_t _status = {.mqtt_sts = MQTT_STS_FREE, .is_building = false};
#endif
...
static void aw_at_urc_table_set(AwCallBack_char_t funcRecv, AwCallBack_char_t funcStat) {
...
static u32 _aw_ms_to_tick(u32 ms) { return (ms * 1000 / Clock_tickPeriod); }
...
#ifdef IS_USE_RV
#define _AW_AT_PAYLOAD_SIZE 1152
#else
#define _AW_AT_PAYLOAD_SIZE 128
#endif
static char _payload[_AW_AT_PAYLOAD_SIZE] = {'\0'};  //字符串缓存,用于数据发送和参数化配置
static at_resp_status_t aw_at_exec_cmd(u32 timeout, const char *__restrict _format, ...) {
    va_list args;
    va_start(args, _format);
...
#endif
    aw_at_response_set_line(AW_MQTT_OK_LINE_NUM);
    at_resp_status_t rsp_sts = aw_at_exec_cmd(AT_SHORT_TIMEOUT, "AT+CEREG?\r\n");
    if (AT_RESP_OK == rsp_sts) {
...
#endif
    aw_at_response_set_line(AW_MQTT_OK_LINE_NUM);
    at_resp_status_t rsp_sts = aw_at_exec_cmd(AT_DEFAULT_TIMEOUT, "" AT_REBOOT "" AT_END_CR_LF "");
#ifdef _IS_USE_STATIS
    _statis.reset_cnt++;
#endif
#ifdef _IS_USE_STATUS
...
static int aw_mqtt_set_will(aw_qos_t qos, u8 retain, const char *msg, const char *topic) {
#ifdef _IS_USE_STATUS
    _status.mqtt_sts = MQTT_STS_CONFIG;
#endif
    aw_at_response_set_line(AW_MQTT_OK_LINE_NUM);
    at_resp_status_t rsp_sts =
        aw_at_exec_cmd(AT_DEFAULT_TIMEOUT, "" AT_MQTT_WILL "" AT_END_CR_LF "", ConnectID_0, qos,
                       retain, topic, msg);
#ifdef _IS_USE_STATUS
    _status.mqtt_sts = MQTT_STS_FREE;
#endif
...
static int aw_mqtt_close() {
    ...
    at_resp_status_t rsp_sts =
        aw_at_exec_cmd(AT_DEFAULT_TIMEOUT, "AT+QMTCLOSE=%d\r\n", ConnectID_0);
    if (AT_RESP_OK == rsp_sts) {
        if (AW_OK !=
            aw_at_response_check_key(AT_MQTT_CLOSE_SUCC, ConnectID_0, AT_MQTT_RES_CODE_OK)) {
            rsp_sts = AT_RESP_ERROR;
        }
    }
#ifdef _IS_USE_STATUS
...
#endif
    aw_at_response_set_line(AW_MQTT_SUCC_LINE_NUM);
    at_resp_status_t rsp_sts = aw_at_exec_cmd(AT_DEFAULT_TIMEOUT, "AT+QMTOPEN=%d,\"%s\",%d\r\n",
                                              ConnectID_0, hostName, port);

    if (AT_RESP_OK == rsp_sts) {
        if (AW_OK != aw_at_response_check_key(AT_MQTT_OPEN_SUCC, ConnectID_0, AT_MQTT_RES_OK)) {
            rsp_sts = AT_RESP_ERROR;
        }
    }
...
static int aw_mqtt_connect(const char *clientID) {
    ...
    if (AT_RESP_OK == rsp_sts) {
        if (AW_OK != aw_at_response_check_key(AT_MQTT_CONNECT_SUCC, ConnectID_0, AT_MQTT_RES_OK,
                                              AT_MQTT_RES_CODE_OK)) {
            rsp_sts = AT_RESP_ERROR;
        }
    }
...
#endif
    aw_at_response_set_line(AW_MQTT_SUCC_LINE_NUM);
    at_resp_status_t rsp_sts = aw_at_exec_cmd(AT_DEFAULT_TIMEOUT, "" AT_MQTT_SUB "" AT_END_CR_LF "",
                                              ConnectID_0, MsgID_1, topic, qos);

    if (AT_RESP_OK == rsp_sts) {
        if (AW_OK !=
            aw_at_response_check_key(AT_MQTT_SUB_SUCC, ConnectID_0, MsgID_1, AT_MQTT_RES_OK, qos)) {
            rsp_sts = AT_RESP_ERROR;
        }
    }
...
static int aw_mqtt_build_net() {
    ...
    res = aw_mqtt_set_will_f(QOS1_LEASET_ONCE, NO_RETAIN,
                             "{" CONNECT_CNT ":0," DEV_TYPE ":" AW_STR(AW_DEV_TYPE) "}",
    ...
    if (_is_sub == 1) {
        res = aw_mqtt_sub_f(QOS1_LEASET_ONCE, "%s/%s/" AW_TOPIC_CMD "", DevName, DevId);
        if (AW_OK != res) {
            return AW_ERR;
        }
    }
...
#endif
    while (AW_OK != aw_mqtt_connect_cnt(10)) {
        aw_delay_ms(300);
    }
#ifdef _IS_USE_STATUS
...
static s8 aw_mqtt_pub(aw_qos_t qos, u8 retain, const char *topic, const char *msg) {
#ifdef _IS_USE_STATUS
    ...
    at_resp_status_t rsp_sts =
        aw_at_exec_cmd(AT_DEFAULT_TIMEOUT, "AT+QMTPUB=%d,%d,%d,%d,\"%s\",%s\r\n", ConnectID_0,
                       msgId, (int)qos, (int)retain, topic, msg);
    if (AT_RESP_OK == rsp_sts) {
        if (AW_OK !=
            aw_at_response_check_key(AT_MQTT_PUB_SUCC, ConnectID_0, msgId, AT_MQTT_RES_OK)) {
            rsp_sts = AT_RESP_ERROR;
        }
    }
    if (AT_RESP_OK == rsp_sts) {
...
static s8 aw_mqtt_pubing(aw_qos_t qos, u8 retain, const char *topic, const char *msg) {
...
int aw_mqtt_pub_f(aw_pub_type_t pub_type, aw_qos_t qos, u8 retain, const char *topic,
                  const char *__restrict _format, ...) {
    int res     = 0;
    _payload[0] = '\0';
    va_list args;
...
int aw_mqtt_pub_flush(aw_pub_type_t pub_type, aw_qos_t qos, u8 retain, const char *topic) {
    int res = 0;
    int len = strlen(_payload);
    if (len < 1) {
        return -2;
...
int aw_mqtt_pub_status() {
    ...
    res = aw_mqtt_pub_f(
        ...
        aw_mqtt_topic_get_f("" AW_MQTT_DEV_NAME "/%s/" AW_STATUS "", AW_MQTT_DEV_ID),
        "{" CONNECT_CNT ":%d," RESET_NB ":%d,cnc:%d,pbe:%d," CSQ ":%d," DEV_TYPE ":%d}",
...
//                   "CSQ:%d",
//                   _statis.connect_ok_cnt, _statis.reset_cnt, _statis.connect_cnt,
//                   _statis.recv_urc_cnt, _statis.lost_urc_cnt, _statis.sub_err_cnt,
//                   _statis.open_err_cnt, _statis.connect_err_cnt, aw_mqtt_get_csq());
//#endif
#else
    res = aw_mqtt_pub_f(AW_PUB_MODAL, QOS0_MOST_ONECE, NO_RETAIN, "sts", "{CSQ:%d}",
                        aw_mqtt_get_csq());
//#ifdef IS_USE_OLED
...
int aw_mqtt_net_building() {
    ...
    return aw_mqtt_pub_f(
        ...
        aw_mqtt_topic_get_f("" AW_MQTT_DEV_NAME "/%s/" AW_NOTIFY "", AW_MQTT_DEV_ID),
        "{" CONNECT_CNT ":%d," RESET_NB ":%d," CSQ ":%d," DEV_TYPE ":%d," AW_VERSION ":\"%s\"}",
        _statis.connect_ok_cnt, _statis.reset_cnt, aw_mqtt_get_csq(), AW_DEV_TYPE,
        AW_firmware_version);
#else
    return aw_mqtt_pub_f(AW_PUB_MODAL, QOS1_LEASET_ONCE, NO_RETAIN, "nty", "{\"CON\":\"%d\"}", 1);
#endif
}
