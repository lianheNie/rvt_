...
static s8 _mqtt_pub_sensor(Aw_Sensor_t *sensor) {
    ...
    if (AW_devtype_rd == sensor->type || AW_devtype_gateway_rd == sensor->type) {
    ...
    } else if (AW_devtype_rp == sensor->type || AW_devtype_gateway_rp == sensor->type) {
        ...
        topic = aw_mqtt_topic_get_f("" AW_MQTT_DEV_NAME "/%s/" AW_NOTIFY "", AW_MQTT_DEV_ID);
    }
    res = aw_mqtt_pub_f(AW_PUB_MODAL, QOS0_MOST_ONECE, NO_RETAIN, topic,
                        "{i:%d,t:%d,y:%d,v:%d,b:%d,r:%d,c:%d,csq:%d}", AW_DEV_SELF_ADDRESS,
                        sensor->src_id, sensor->type, sensor->val, sensor->bat, sensor->rssi,
                        sensor->cnt, aw_mqtt_get_csq());
...
static int _rvt_report_time = 300;  //上传周期
#endif
...
#include "aw_json.h"
static JsonBuf_t _json_buf = {.head = 0, .tail = 0, .next = 1, .size = AW_JSON_BUF_SIZE};
...
static void _mqtt_pub_status_data() {
    ...
    aw_mqtt_pub_f(AW_PUB_MODAL, QOS0_MOST_ONECE, NO_RETAIN,
                  ...
                  "{i:%d,y:%d,b:%d,bc:%d,bv:%d,byv:%d,bs:%d,cc1:%d,cc2:%d,cc3:%d,c1:%d,c2:%d,c3:%d,"
                  "csq:%d,ver:%d}",
                  AW_DEV_SELF_ADDRESS, AW_DEV_TYPE, bat, bat_c, bat_v, sys_v, bus_v, cur_12v,
                  cur_5v, cur_4v, chs[AW_SW_CH1], chs[AW_SW_CH2], chs[AW_SW_CH3], aw_mqtt_get_csq(),
                  AW_version_num);
#elif AW_devtype_rs_3 == AW_DEV_TYPE
    aw_mqtt_pub_f(AW_PUB_MODAL, QOS0_MOST_ONECE, NO_RETAIN,
                  aw_mqtt_topic_get_f("" AW_MQTT_DEV_NAME "/%s/" AW_STATUS "", AW_MQTT_DEV_ID),
                  "{i:%d,y:%d,c1:%d,c2:%d,c3:%d,csq:%d,ver:%d}", AW_DEV_SELF_ADDRESS, AW_DEV_TYPE,
                  chs[AW_SW_CH1], chs[AW_SW_CH2], chs[AW_SW_CH3], aw_mqtt_get_csq(),
                  AW_version_num);
#elif AW_devtype_gateway_rs_np_1 == AW_DEV_TYPE || AW_devtype_rs_np_1 == AW_DEV_TYPE || \
    AW_devtype_rs_1 == AW_DEV_TYPE
    aw_mqtt_pub_f(AW_PUB_MODAL, QOS0_MOST_ONECE, NO_RETAIN,
                  aw_mqtt_topic_get_f("" AW_MQTT_DEV_NAME "/%s/" AW_STATUS "", AW_MQTT_DEV_ID),
                  "{i:%d,y:%d,c1:%d,csq:%d,ver:%d}", AW_DEV_SELF_ADDRESS, AW_DEV_TYPE,
                  chs[AW_SW_CH1], aw_mqtt_get_csq(), AW_version_num);
#else
    aw_mqtt_pub_f(AW_PUB_MODAL, QOS0_MOST_ONECE, NO_RETAIN,
                  aw_mqtt_topic_get_f("" AW_MQTT_DEV_NAME "/%s/" AW_STATUS "", AW_MQTT_DEV_ID),
                  "{i:%d,y:%d,csq:%d,rpt:%d,sip:%s,ver:%d}", AW_DEV_SELF_ADDRESS, AW_DEV_TYPE,
                  aw_mqtt_get_csq(), _rvt_report_time, aw_mqtt_get_host(), AW_version_num);
#endif
}
static void _proc_mqtt_recv_event() {                   // mqtt 数据接收处理事件
    while (true == read_json(&_json_buf, _json_str)) {  //读json队列
        int len = strlen(_json_str);
        if (len > 0) {
            aw_mqtt_pub_f(AW_PUB_MODAL, QOS0_MOST_ONECE, NO_RETAIN,
                          aw_mqtt_topic_get_f("" AW_MQTT_DEV_NAME "/%s/" AW_ACK "", AW_MQTT_DEV_ID),
                          "{ack:%s,cnt:%d,csq:%d,y:%d,ver:%d}", _json_str,
                          (NULL != _mqtt_statis) ? _mqtt_statis->recv_urc_cnt : 0,
                          aw_mqtt_get_csq(), AW_DEV_TYPE, AW_version_num);  //发送命令应答

            char *pStr      = NULL;
            int   val       = 0;
            int   select_id = -1;
            if (pStr = strstr(_json_str, SELECT_ID)) {  //设备ID
                if (sscanf(pStr - 1, "\"" SELECT_ID "\":%d", &val) == 1) {
                    select_id = val;
                }
            }
            int cnt = 0;
            aw_mqtt_pub_add_f("{i:%d", select_id > 0 ? select_id : AW_DEV_SELF_ADDRESS);
            cnt++;
            ...
            if (pStr = strstr(_json_str, GET_VIBR)) {  //获取振动数据
                if (sscanf(pStr - 1, "\"" GET_VIBR "\":%d", &val) == 1) {
                    ...
                    aw_vibration_process();  //读取振动数据并发送
                }
            }
            if (pStr = strstr(_json_str, VIBR_BIAS)) {  //振动传感器校准
                if (sscanf(pStr - 1, "\"" VIBR_BIAS "\":%d", &val) == 1) {
                    ...
                    if (cal) {
                        aw_mqtt_pub_add_f("\"" VIBR_BIAS "\":[%d,%d,%d]", cal[0], cal[1], cal[2]);
                    }
                    cnt++;
                }
            }
...
#endif
            if (pStr = strstr(_json_str, SET_NET_LED)) {  // NB模块网络灯配置
            ...
            if (pStr = strstr(_json_str, ATE)) {  // NB模块回显设置
...
#if defined(IS_USE_LS) || (defined(IS_USE_RS) && defined(IS_USE_EASYLINK))  //启用开关模块
            int        _rs_delay          = 0;
            ...
            if (pStr = strstr(_json_str, RESET_NB)) {  //复位NB模块
            ...
            if (pStr = strstr(_json_str, RESET_CPU)) {  //复位单片机
...
static void _mqtt_timer_init() {
    Timer_construct(&_mqtt_clockS, _mqtt_timeout_cb, _MQTT_TIMEOUT, _MQTT_TIMEOUT, false, 0);
...
#ifdef IS_USE_RVT_TIMER
#include "util_timer.h"
#define _RVT_TIME_BASE 60                     //基准周期300s
...
static void _rvt_timer_init() {
    Timer_construct(&_rvt_clockS, _rvt_timeout_cb, _RVT_TIMEOUT, _RVT_TIMEOUT, false, 0);
    Timer_start(&_rvt_clockS);  //开定时器
}
...
static void _proc_rvt_timer_event() {
    ...
    if (_is_key_set && uart_is_open())  //配置按键是否按下
    {
...
#endif
    // read temperatuer
    _process_sensor(AW_devtype_rt, tempt, ++_rvt_timer_cnt);  //读取温度数据，并发送

    // read acc sensor
    aw_vibration_process();  //读取振动数据，并发送
...
#endif
    if (_events & MQTT_RECV_EVENT) {  // mqtt 数据接收事件
...
#if defined(IS_USE_RT) || defined(IS_USE_RV)
#ifdef IS_USE_RVT_TIMER
    if (_events & RVT_TIMER_EVENT) {  //温度振动传感器定时事件
...
#if defined(IS_USE_LS)
#if AW_devtype_gateway_rs_np_1 == AW_DEV_TYPE || AW_devtype_rs_np_1 == AW_DEV_TYPE
